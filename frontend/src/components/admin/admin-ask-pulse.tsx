 ...
 *
 * Responses are generated by the backend via Gemini (with rule-based fallback).
 */
...
export function AdminAskPulse({
  allPatients: _allPatients,
  waitingPatients: _waitingPatients,
}: AdminAskPulseProps) {
  ...
  const send = useCallback(
    async (text: string) => {
      ...
      setThinking(true);
      try {
        const response = await askPulse(text.trim(), "admin");
        const aiMsg: Message = {
        ...
        setMessages((prev) => [
          ...prev,
          {
            id: String(Date.now() + 1),
            role: "ai",
            text: "Could not reach Pulse right now. Please try again.",
          },
        ]);
      } finally {
        setThinking(false);
          ...
          >
            <div className="flex items-center" style={{ gap: 7 }}>
              <span
                ...
                aria-hidden="true"
              />
              <span style={{ fontSize: 13, fontWeight: 700, color: "#7C5FAE" }}>
                Ask Pulse
              </span>
              <span
                style={{
          ...
          >
            {messages.length === 0 && (
              <>
                ...
                >
                  Ask me anything about patients, departments, discharges, or bed
                  availability — I&apos;ll pull from live data.
                </p>
            ...
            {thinking && (
              <div
                ...
                style={{ gap: 7, color: "#8A8394", fontSize: 12 }}
              >
